...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://job-task-2-backend.vercel.app/api/v1/auth/login', {
        method: 'POST',
        headers: {
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-1 gap-3">
              <div className="text-center text-sm text-gray-600">
                <p>Use any valid email and password combination</p>
                <p className="mt-1 text-xs">Backend will authenticate against its database</p>
              </div>
            </div>
          </div>
        </div>
